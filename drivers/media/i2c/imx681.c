...
#define IMX681_XCLK_RATE	19200000
...
static int imx681_parse_endpoint(struct imx681 *sensor)
{
	...
	if (ep.bus_type != V4L2_MBUS_CSI2_CPHY) {
		ret = dev_err_probe(sensor->dev, -EINVAL,
				    "only CSI-2 C-PHY is supported\n");
		goto free_endpoint;
	}

	if (ep.bus.mipi_csi2.num_data_lanes != 1 ||
	    ep.bus.mipi_csi2.data_lanes[0] != 0) {
		ret = dev_err_probe(sensor->dev, -EINVAL,
				    "only C-PHY trio 0 is supported\n");
		goto free_endpoint;
	}

	if (ep.nr_of_link_frequencies != 1 ||
	    ep.link_frequencies[0] != IMX681_LINK_FREQ) {
		ret = dev_err_probe(sensor->dev, -EINVAL,
...
static int imx681_probe(struct i2c_client *client)
{
	...
	if (IS_ERR(sensor->xclk))
		return dev_err_probe(dev, PTR_ERR(sensor->xclk),
				     "failed to get xclk\n");
	if (clk_get_rate(sensor->xclk) != IMX681_XCLK_RATE)
		return dev_err_probe(dev, -EINVAL,
				     "xclk must be 19.2 MHz\n");
	...
	ret = imx681_parse_endpoint(sensor);
	if (ret)
		return ret;
